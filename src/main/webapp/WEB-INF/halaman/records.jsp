...
<c:url var="addUrl" value="/main/add" />
<table style="border: 1px solid; width: 100%; text-align:center">
	...
	<c:forEach items="${persons}" var="person">
		<c:url var="editUrl" value="/main/edit?id=${person.id}" />
		<c:url var="deleteUrl" value="/main/delete?id=${person.id}" />
		...
		<c:if test="${!empty person.creditCards}">
			...
			<tr>
				...
				<c:url var="addCcUrl" value="/creditcard/add?id=${person.id}" />
				<c:url var="editCcUrl" value="/creditcard/edit?pid=${person.id}&cid=${creditCard.id}" />
				<c:url var="deleteCcUrl" value="/creditcard/delete?id=${creditCard.id}" />
